...
if (request.getAttribute("mainQuestion") == null || request.getAttribute("mainCurrentQuestionNumber") == null
...
<body>
	...
	<div class="mainpage-main-container"
		...
		<div class="mainpage-section-searchjob">
			...
			<div
				style="font-size: 14px; color: #4E77FF; text-align: center; font-weight: 600; background: rgba(78, 119, 255, 0.08); padding: 10px 16px; border-radius: 999px;">
				질문에 대한 답변은 회원만 받아 볼 수 있습니다.</div>

			<div class="mainpage-searchjob-question" id="question">
				<c:choose>
					<c:when test="${not empty mainQuestion}">
            Q${mainQuestion.jobQuestionNumber}. <c:out
							value="${mainQuestion.jobQuestionContent}" />
					</c:when>
					<c:otherwise>
			...
			<form id="mainQuestionForm"
				...
				<button type="submit" id="mainpage-hidden-submit"
					style="display: none;"></button>

				<div class="mainpage-searchjob-searchbar">
					<input type="text" name="answer" id="mainpage-searchjob-answer"
						placeholder="답변을 입력해주세요" maxlength="4000"> <img
						src="${pageContext.request.contextPath}/assets/img/검색돋보기.png"
						alt="검색" id="searchimg">
				</div>
			</form>
		</div>
				...
				<c:choose>
						...
						<c:forEach var="jobGroup" items="${mainJobGroupList}">
							...
							<c:choose>
								<c:when test="${jobGroup.jobNumber == 1}">
									...
									<c:set var="jobSummary"
										value="고객의 요청 대상에 취약점 진단 컨설팅을 수행하여 최신의 해킹 및 침해 형태 변화에 따른 대응을 강화하고, 지속적으로 최상의 보안 상태를 유지 및 관리할 수 있도록 진단 컨설팅을 제공" />
								</c:when>

								<c:when test="${jobGroup.jobNumber == 2}">
									<c:set var="jobIconPath" value="시스템 네트워크 엔지니어 마크.png" />
									<c:set var="jobSummary"
										value="보안 시스템을 설계하고 구축하며, 개발과 인프라 환경에 보안을 적용하는 계열" />
								</c:when>

								<c:when test="${jobGroup.jobNumber == 3}">
									<c:set var="jobIconPath" value="보안관제.png" />
									<c:set var="jobSummary"
										value="조직의 정보 자산 보호를 위한 24시간 365일 사이버 위협 대응 및 사전 예방을 담당하는 계열" />
								</c:when>

								<c:when test="${jobGroup.jobNumber == 4}">
									<c:set var="jobIconPath" value="침해 사고 디지털 포렌식.png" />
									<c:set var="jobSummary"
										value="해킹 악성코드 등 정보시스템 침해 시 원인과 경로를 규명하고 증거의 무결성을 확보하여 분석하는 분야" />
								</c:when>
							</c:choose>

							<a
								href="${pageContext.request.contextPath}/roadmap/detail.rfc?jobNumber=${jobGroup.jobNumber}"
					...
					<c:otherwise>
						<div class="mainpage-jobcategory-card">
							...
							<div class="mainpage-jobcategory-cardcontent">아직 등록된 추천 직군
								유형이 없습니다.</div>
							<div class="mainpage-jobcategory-cardbottom">
								<div class="mainpage-jobcategory-cardbottomtext">관리자 등록
									대기중</div>
							</div>
						</div>
					</c:otherwise>
				...
				<c:choose>
					<c:when test="${not empty mainCompanyList}">
						<c:forEach var="company" items="${mainCompanyList}">
							...
							<c:if test="${not empty company.filePath}">
								<c:choose>
									<c:when
										test="${fn:startsWith(company.filePath, 'http://') or fn:startsWith(company.filePath, 'https://')}">
										<c:set var="companyImgSrc" value="${company.filePath}" />
									</c:when>
									<c:when test="${fn:startsWith(company.filePath, '/')}">
										<c:set var="companyImgSrc"
											value="${pageContext.request.contextPath}${company.filePath}" />
									</c:when>
									<c:otherwise>
										<c:set var="companyImgSrc"
											value="${pageContext.request.contextPath}/${company.filePath}" />
					...
					<c:otherwise>
						<div class="mainpage-companyad-companycard">
							<img src="${defaultCompanyImg}" alt="기본 이미지"
								class="mainpage-companyad-companyimg">
							<div class="mainpage-companyad-companycardtext">
								<div class="mainpage-companyad-companyname">기업 정보 준비중</div>
								<div class="mainpage-companyad-companysummary">노출 가능한 기업
									홍보 정보가 아직 없습니다.</div>
		...
		<div class="mainpage-community-cards">
			<a
				href="${pageContext.request.contextPath}/app/main/community/post-list.jsp"
			...
			</a> <a href="${pageContext.request.contextPath}/qna/list.qfc"
				class="mainpage-community-card">
				<div class="mainpage-community-cardtop">
			...
			<div class="mainpage-community-calendarcard">
				<div class="mainpage-community-calendarhead">
					...
					<a
						href="${pageContext.request.contextPath}/app/main/expo/expo-calendar.jsp"
